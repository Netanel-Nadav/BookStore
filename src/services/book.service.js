...
const STORAGE_KEY1 = 'books'
const STORAGE_KEY2 = 'favorites'
...
async function query() {
    const books = await storageService.query(STORAGE_KEY1)
    if (books.length) {
        return books
    }
    else {
        ...
        await storageService.postMany(STORAGE_KEY1, gBooks)
        return gBooks
    }
}

async function update(book) {
    await storageService.put(STORAGE_KEY1, book)
    return Promise.resolve()
}

function getById(bookId) {
    return storageService.get(STORAGE_KEY1, bookId)
}

async function queryFavorites(sortBy = null) {
    let books = await query()
    let favorites = books.filter(book => book.isFavorite === true)
    if (sortBy) favorites = _sortFavorites(favorites, sortBy)
    const prices = favorites.map(book => +book.price)
    const totalPrice = (prices.reduce((a, b) => a + b, 0)).toFixed(1)
    return Promise.resolve({ favorites, totalPrice })
}

function saveToFavorites(book) {
    return storageService.post(STORAGE_KEY2, book)
}

function removeFromFavorites(bookId) {
    return storageService.remove(STORAGE_KEY2, bookId)
}

function _sortFavorites(books, sortBy) {
