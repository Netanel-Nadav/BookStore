...
function queryFavorites(sortBy = null) {
    let favorites = storageService.query(STORAGE_KEY2)
    if (favorites.length) {
        if (sortBy) favorites = _sortFavorites(favorites, sortBy)
        const prices = favorites.map(book => book.price)
        const totalPrice = prices.reduce((a, b) => a + b, 0)
        return {favorites, totalPrice}
    }
}
